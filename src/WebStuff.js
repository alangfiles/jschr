...
WebStuff.changeColors = function() {
  //array of 64, and I want distinct

  const colors = [];

  while (colors.length < 4) {
    const num = Math.floor(Math.random() * 56);
    if (colors.indexOf(num) == -1) {
      colors.push(num);
    }
  }
  WebStuff.changeColor("a", colors[0]);
  WebStuff.changeColor("b", colors[1]);
  WebStuff.changeColor("c", colors[2]);
  WebStuff.changeColor("d", colors[3]);
};
