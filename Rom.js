...
Rom.prototype = Object.assign(Object.create(Memory.prototype), {
  ...
  dumpCHRData: function() {
    return this.header.getCHRROMBanksNum();
  },
...
Object.assign(RomHeader.prototype, {
   ...
   */
  isNes: function() {
    if (this.getSignature() !== this.VALID_SIGNATURE) return false;

    if (this.getMagicNumber() !== this.VALID_MAGIC_NUMBER) return false;
   ...
   */
  getMirroringType: function() {
    return this.extractBits(
      this.getControlByte1(),
      this.MIRRORING_TYPE_BIT,
      this.MIRRORING_TYPE_BITS_WIDTH
    );
  },
  ...
  getBatteryBackedRAM: function() {
    return this.extractBits(
      this.getControlByte1(),
      this.BATTERY_BACKED_RAM_BIT,
      this.BATTERY_BACKED_RAM_BITS_WIDTH
    );
  },
  ...
  getTrainer512Bytes: function() {
    return this.extractBits(
      this.getControlByte1(),
      this.TRAINER_512BYTES_BIT,
      this.TRAINER_512BYTES_BITS_WIDTH
    );
  },
  ...
  getFourScreenMirroring: function() {
    return this.extractBits(
      ...
      this.FOUR_SCREEN_MIRRORING_BITS_WIDTH
    );
  },
  ...
  getMapperNum: function() {
    var lowerBits = this.extractBits(
      this.getControlByte1(),
      this.MAPPER_LOWER_BIT,
      this.MAPPER_LOWER_BITS_WIDTH
    );
    var higherBits = this.extractBits(
      this.getControlByte2(),
      this.MAPPER_HIGHER_BIT,
      this.MAPPER_HIGHER_BITS_WIDTH
    );
  ...
  dump: function() {
    ...
    buffer +=
      "Magic Number: " +
      Utility.convertDecToHexString(this.getMagicNumber(), 2) +
      "\n";
    buffer +=
      "PRG-ROM banks num: " +
    ...
    buffer +=
      "Control1: " +
      Utility.convertDecToHexString(this.getControlByte1(), 2) +
      "\n";
    buffer +=
      "Control2: " +
      Utility.convertDecToHexString(this.getControlByte2(), 2) +
      "\n";
    buffer +=
      "RAM banks num: " +
      Utility.convertDecToHexString(this.getRAMBanksNum(), 2) +
      "\n";
    buffer +=
      "Unused field: " +
      Utility.convertDecToHexString(this.getUnusedField(), 14) +
      "\n";
    buffer += "\n";
    buffer += "In control bytes\n";
    buffer +=
      "Mirroring type: " +
      Utility.convertDecToHexString(this.getMirroringType()) +
      "(" +
    ...
    buffer +=
      "Mapper number: " +
      Utility.convertDecToHexString(this.getMapperNum(), 2) +
      "(" +
